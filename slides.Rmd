...
- Spatial distribution: how do sales vary by post code, calibrated for population
...
background-position: center top
class: left
# Postal codes by quantity dispensed 

Counts by postcode adjusted to per person, using Census population numbers. Low quantities (<20) tends to be rural, medium (20-100) towards the east coast concentrating in NSW, and a few locations have massive numbers dispensed (more than one thousand per person) - 2015 totals
...
<img src="map_quantity.png" width="800">

```{r warning=FALSE, fig.width=7, fig.height=2}
postcode_id <- distinct(nat_map, POA_CODE, id)

spend_pop_postcode %>%
right_join(postcode_id, by=c("postcode"="POA_CODE")) %>%
ggplot(aes(fill=ifelse(is.na(Qnt_Pop),0,Qnt_Pop))) + coord_fixed() +
...
Highest amounts spent per person in a postcode was close to $2000, and in south-east Australia.

<img src="map_amount.png" width="800">

```{r warning=FALSE, fig.width=7, fig.height=2}
mp <- spend_pop_postcode %>%
  right_join(postcode_id, by=c("postcode"="POA_CODE")) 
p1 <- ggplot(data=mp, aes(fill=ifelse(is.na(Amt_Pop),0,Qnt_Pop))) + 
  coord_fixed() +
    geom_map(map=nat_map_pop,aes(map_id=id)) +
    #scale_y_continuous(limits=c(-38.5,-37.5),oob=function(a,b)a) +
...
# Decrease in osteoporosis drugs primarily 

.pull-left[
due to women, and increase in urology drugs primarily due to men. Women are the primary recipients of drugs for depression, hypertension, and contribute to increase in immunology drugs.
]
...
cogs_df %>%
  ...
  ggtitle("Cognostics for each postcode over time") -> p1
...
dispense_chronic_bypostcode_qtr %>% 
  ...
  xlab("Dispense quarter") +
  ylab("The number of transactions") +
  ggtitle("The most increasing postcode over time") -> p2
grid.arrange(p1, p2, ncol=2)
```

Time series cognostics measure aspects such as overall variability (entropy), linear trend, seasonality and spikiness. These are computed for each postcode, giving them a rating on these sorts of patterns.
---
# Digging out some postcodes of interest?
...
dispense_chronic_bypostcode_qtr %>% 
  ...
  xlab("Dispense quarter") +
  ylab("The number of transactions") +
  ggtitle("The most predictable postcode over time") -> p1
...
dispense_chronic_bypostcode_qtr %>% 
  ...
  xlab("Dispense quarter") +
  ylab("The number of transactions") +
  ggtitle("The most spiky postcode over time") -> p2
...
dispense_chronic_bypostcode_qtr %>% 
  ...
  xlab("Dispense quarter") +
  ylab("The number of transactions") +
  ggtitle("The most seasonal postcode over time") -> p3
...
These are their locations. And the top spending was on drugs for epilepsy, and depression. 
```{r fig.width=10, fig.height=6}
...
# Drug prescriptions for postcode by 
expected market share. Top 15 greater than expected. There is some evidence of prescribers making drug preferences for their patients.
...
SubsetFavoured %>% arrange(desc(ratio)) %>% 
  select(ManufacturerName, postcode, ratio) %>% head(15)
#ggplot(SubsetFavoured) + 
 # geom_point(aes(postcode, ratio,
 #                colour=ManufacturerName)) +
 # labs(y='Ratio of Total : Expected Revenue')
```
...
All analysis was done using [R](http://https://cran.r-project.org), using the packages `knitr`, `zoo`, `RSQLite`, `forcats`, `tidyverse`, `lubridate`, `gridExtra`, `ggthemes`, `viridis` and their dependencies.
