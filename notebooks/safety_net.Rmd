...
df <- records %>%
    ...
    ungroup() %>%
    mutate(
        safety_net=factor(ifelse(safety_net,"Safety net","Co-payment"),
            c("Safety net", "Co-payment")),
        concession=factor(ifelse(concession,"Concession","Non-concession"))
    )
```

## A striking yearly pattern in purchases over time
...
Non-concession card holders currently pay \$38.80 per purchase but nothing beyond a total of \$1,494.90 each year.

Slightly more may be paid for brands more expensive than the cheapest for a drug.

I've inferred concession card holding, for each patient in each year, if a patient has purchased medicine with wholesale price more than \$10 but where the government has made a contribution the patient has always paid less than \$10.

```{r}
df %>%
#mutate(Dispense_Week = floor_date(Dispense_Week, unit="6 months") %m+% months(3)) %>%
count(Dispense_Week, safety_net, concession) %>%
ungroup() %>%
ggplot(aes(x=Dispense_Week, y=n, fill=safety_net)) + geom_col(width=7) +
...
count(Dispense_Week, safety_net, concession) %>%
ungroup() %>%
ggplot(aes(x=Dispense_Week, y=n, fill=safety_net)) + geom_col(width=100) +
    facet_grid(concession ~ .) + 
    labs(fill="", x="", y="Number of prescription purchases")
```
...
#mutate(Dispense_Week = floor_date(Dispense_Week, unit="6 months") %m+% months(3)) %>%
count(illness, Dispense_Week, safety_net, concession) %>%
ungroup() %>%
ggplot(aes(x=Dispense_Week, y=n, fill=safety_net)) + geom_col(width=7) +
...
## Where is the safety net used?

```{r}
p <- df %>%
    group_by(postcode) %>%
    summarize(n=n(), prop_safety_net=mean(safety_net == "Safety net")) %>%
    filter(n >= 100) %>%
